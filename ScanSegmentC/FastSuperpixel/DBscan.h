    ...
    int Cols, unsigned short* label, double* labelsL, double* labelsa, double* labelsb, double* labelsx, double* labelsy)
{
    ...
    ngt = static_cast<int*>(malloc(100 * sizeof(int)));
    //     lab= new int [100];
    ngt1 = static_cast<int*>(malloc(100 * sizeof(int)));
    ...
    for (int j = 0; j < k; j++)
    {
        n = ngt[j];
        bl = 1;
        if (n + 1 < pixelnum && label[n + 1] == label[n])
        {
            for (int i = 0; bl == 1; i++)
            {
                if (ngt[i] == n + 1) bl = 0;
        ...
        bl = 1;
        if (n + Cols < pixelnum && label[n + Cols] == label[n])
        {
            for (int i = 0; bl == 1; i++)
            {
...
void DBscan(unsigned char* R, unsigned char* G, unsigned char* B, int Rows, int Cols, unsigned short* label, double supnumber, int& realnumber, int post)
{
    ...
    unsigned int* labnumb = static_cast<unsigned int*>(malloc(pixel * sizeof(unsigned int))); //number pixels in superpixel
    int Nc = 0, nb;
    bool* label0 = static_cast<bool*>(malloc(pixel * sizeof(bool)));
    for (int i = 0; i < pixel; i++)label0[i] = 0;

    int* ngbCL = static_cast<int*>(malloc(pixel * sizeof(int)));
    int** ngbC = static_cast<int**>(malloc(pixel * sizeof(int*))); //record detail pixel in each superpixel 
    ...
    int neighboursL = supnumber * 2;
    int* neighbours = static_cast<int*>(malloc(neighboursL * sizeof(int)));
    for (int i = 0; i < neighboursL; i++)neighbours[i] = 0;
    int numbngb;
...
//                 ngbC[Nc][num]=nb;
            ...
            ngbC[Nc] = static_cast<int*>(malloc(num * sizeof(int)));
            ngbCL[Nc] = num;
            ngbC[Nc][0] = i;
            for (int k = 0; k < ind; k++)
            {
    ...
    int Nclength = Nc + 1;
    double* labelsR = static_cast<double*>(malloc(Nclength * sizeof(double)));
    double* labelsG = static_cast<double*>(malloc(Nclength * sizeof(double)));
    double* labelsB = static_cast<double*>(malloc(Nclength * sizeof(double)));
    double* labelsx = static_cast<double*>(malloc(Nclength * sizeof(double)));
    double* labelsy = static_cast<double*>(malloc(Nclength * sizeof(double)));
    int t;
    double labels;
    ...
    int* neighb = static_cast<int*>(malloc(pixel * sizeof(int)));
    ...
    for (int n = 1; n <= Nc; n++)
    {
        if (labnumb[n] < supnumber && labnumb[n]>0)
        {
            ...
            if (minnum != 0)
            {
                ...
                free(ngbC[minnum]);
                ngbCL[minnum] = labnumb[minnum] + labnumb[n];
                ngbC[minnum] = static_cast<int*>(malloc(ngbCL[minnum] * sizeof(int)));
